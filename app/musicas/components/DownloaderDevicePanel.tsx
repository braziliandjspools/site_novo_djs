    ...
    <div id="downloader-panel" className={`min-w-0 rounded-lg border border-zinc-800 bg-[#121212] p-3 ${className}`}>
      ...
      {loading && devices.length === 0 ? (
        ...
        <div className="mt-2 min-w-0 space-y-1">
          <p className="min-w-0 break-words text-xs text-zinc-300">
            <span className="mr-1 inline-block h-2 w-2 flex-shrink-0 rounded-full bg-[#1ed760]" />
        ...
        <div className="mt-2 min-w-0 space-y-2">
          ...
          <select
            ...
            className="w-full min-w-0 max-w-full rounded-md border border-zinc-700 bg-black px-2 py-1.5 text-xs text-white"
          >
      ...
      {totalQueueCount > 0 && (
        <button
          ...
          disabled={clearing}
          className={`mt-3 inline-flex w-full min-w-0 max-w-full items-center justify-center gap-2 rounded-md border px-2 py-1.5 text-[11px] font-semibold transition-colors disabled:opacity-50 ${
            confirmClear
