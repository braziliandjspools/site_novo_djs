...
export function VipMusicTrackList({
  ...
  const handleToggle = useCallback(
    ...
    [canPlay, folderId, selectionMode, toggleTrack],
  );
  ...
  const handleSendToDownloader = useCallback(
    async (track: PreviewTrack) => {
      ...
      try {
        await sendTrackToDownloader(track, relativePath);
        showToast("Adicionado ao Brazilian Packs Downloader");
      } catch (err) {
    ...
    [batchSending, ensureDownloaderAccess, relativePath, sendingId, showToast],
  );
  ...
  const handleSendSelectedToDownloader = useCallback(async () => {
    ...
    try {
      await sendTracksToDownloaderBatch(selectedTracks, relativePath);
      const count = selectedTracks.length;
      showToast(
        count === 1
          ? "Adicionado ao Brazilian Packs Downloader"
          : `${count} faixas adicionadas ao Brazilian Packs Downloader`,
      );
